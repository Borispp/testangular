...
gulp.task 'index', ->
    sources = gulp.src mainBowerFiles().concat([config.paths.built.styles.all, config.paths.built.scripts.all])
    gulp.src config.paths.built.index.file
        .pipe inject sources
        .pipe gulp.dest config.paths.built.index.path
